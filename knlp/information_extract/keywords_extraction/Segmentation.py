...
class SentenceSegmentation(object):
    ...
    def segment(self, text):
        res = [util.as_text(text)]  # return UTF-8 编码

        for sep in self.delimiters:
            # sep is symbol for delimiters
            text, res = res, []
            for seq in text:
                # text is a paggage seq is a paragraph
                res += seq.split(sep)
                # split each para into sentence. add to res . the result is the same as extend
